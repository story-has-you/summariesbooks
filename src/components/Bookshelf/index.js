    ...
    return chunks.map((chunk, index) => {
      return (
        <div
          key={index + 1}
          id={index + 1}
          className="carousel-item w-full grid grid-cols-2 md:grid-cols-5"
        >
          {chunk.map((book) => {
            return (
              <div key={book.id}>
          ...
          <div className="absolute flex justify-between transform -translate-y-1/2 -left-10 -right-10 top-1/2">
            ...
            <a
              ...
              className="btn btn-circle"
              onClick={() => {
                if (Math.floor(count / 15) + 1 > page) setPage(page + 1);
              }}
            >
              ❯
            </a>
    ...
    <>
      <section className="px-6 max-w-6xl mx-auto md:px-0">
        <div className="w-full text-xs py-5 relative">
          <div className="carousel w-full">{bookshelf(books)}</div>
        </div>
      </section>
    </>
  );
